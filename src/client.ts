...
export interface ClientAutoOptions {
    csr: ArrayBuffer | Buffer | String;
    challengeCreateFn: (authz: Authorization, challenge: Challenge, keyAuthorization: string) => Promise<any>;
    challengeRemoveFn: (authz: Authorization, challenge: Challenge, keyAuthorization: string) => Promise<any>;
    email?: string;
    termsOfServiceAgreed?: boolean;
...
export class AcmeClient {
    opts: ClientOptions;
    backoffOpts: { attempts: number | undefined; min: number | undefined; max: number | undefined; };
    api: HttpClient;
     ...
     * ```
     */
    async createAccount(data: AccountCreateRequest = {
        ...
        try {
            ...
            if (resp.status === 200) {
                ngx.log(ngx.INFO, 'njs-acme: [client] Account already exists (HTTP 200), updating it...');
                return await this.updateAccount(data);
            }
     ...
     * ```
     */
    async updateAccountKey(newAccountKey: CryptoKey | string | Buffer, data = {}) {
        ...
        const newHttpClient = new HttpClient(this.opts.directoryUrl, newCryptoKey, accountUrl);
    ...
    async getChallengeKeyAuthorization(challenge) {
        ...
        var keysum = require('crypto').createHash('sha256').update(JSON.stringify(jwk));
    ...
    async waitForValidStatus(item) {
        ...
        ngx.log(ngx.INFO, `njs-acme: [client] Waiting for valid status from: ${item.url} ${this.backoffOpts}`);
 ...
 */
async function auto(client: AcmeClient, userOpts: ClientAutoOptions): Promise<NjsByteString> {
    ...
    try {
        ...
        ngx.log(ngx.INFO, 'njs-acme: [auto] Account URL already exists, skipping account registration');
    }
    catch (e) {
    ...
    ngx.log(ngx.INFO, 'njs-acme: [auto] Parsing domains from Certificate Signing Request');
    ...
    ngx.log(ngx.INFO, `njs-acme: [auto] Resolved ${uniqueDomains.length} unique domains from parsing the Certificate Signing Request`);
     ...
     */
    ngx.log(ngx.INFO, 'njs-acme: [auto] Resolving and satisfying authorization challenges');
    ...
    const challengePromises = authorizations.map(async (authz) => {
        ...
        if (authz.status === 'valid') {
            ngx.log(ngx.INFO, `njs-acme: [auto] [${d}] Authorization already has valid status, no need to complete challenges`);
            return;
        }
        ...
        try {
            ...
            ngx.log(ngx.INFO, `njs-acme: [auto] [${d}] Found ${authz.challenges.length} challenges, selected type: ${challenge.type}`);
            ...
            try {
                ...
                ngx.log(ngx.INFO, `njs-acme: [auto] [${d}] Completing challenge with ACME provider and waiting for valid status`);
                ...
                try {
                    ...
                    ngx.log(ngx.INFO, `njs-acme: [auto] [${d}] challengeRemoveFn threw error: ${e.message}`);
                }
            }
        }
        catch (e) {
            ...
            if (!challengeCompleted) {
                ngx.log(ngx.INFO, `njs-acme: [auto] [${d}] Unable to complete challenge: ${e.message}`);
                ...
                try {
                    ...
                    ngx.log(ngx.INFO, `njs-acme: [auto] [${d}] Authorization deactivation threw error: ${f.message}`);
                }
            }
