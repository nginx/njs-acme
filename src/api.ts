import { formatResponseError, getPublicJwk, RsaPublicJwk, EcdsaPublicJwk } from "./utils"
import { version } from '../package.json';
import { ClientExternalAccountBindingOptions } from "./client";


export type AcmeMethod = "GET" | "HEAD" | "POST" | "POST-as-GET";
export type AcmeResource = "newNonce" | "newAccount" | "newAuthz" | "newOrder" | "revokeCert" | "keyChange" | "renewalInfo"
...
export class HttpClient {
    ...
    */
    constructor(directoryUrl: string, accountKey: CryptoKey, accountUrl: string = "", externalAccountBinding: ClientExternalAccountBindingOptions = {
        kid: "",
        hmacKey: ""
    }) {
     ...
     */
    async request(url: NjsStringLike, method: AcmeMethod, body: NjsStringLike = "") {

        let options: NgxFetchOptions = {
            headers: {
        ...
        /* Request */
        if (this.debug) {
            ngx.log(ngx.INFO, `njs-acme: [http] Sending a new request: ${method} ${url} ${JSON.stringify(options)}`);
        }
        const resp = await ngx.fetch(url, options);
        if (this.debug) {
            ngx.log(ngx.INFO, `njs-acme: [http] Got a response: ${resp.status} ${method} ${url} ${JSON.stringify(resp.headers)}`);
        }
        return resp;
    }
     ...
     */
    async signedRequest(url: string, payload: object, { kid = null, nonce = null, includeExternalAccountBinding = false } = {}, attempts = 0): Promise<Response> {
        if (!nonce) {
        ...
        if (this.debug) {
            ngx.log(ngx.INFO, `njs-acme: [http] Signing request with kid: ${kid}  nonce: ${nonce} jwt: ${JSON.stringify(this.jwk)}`);
        }
        ...
        */
        if (includeExternalAccountBinding && this.externalAccountBinding) {
            if (this.externalAccountBinding.kid && this.externalAccountBinding.hmacKey) {
                ...
                // FIXME
                (payload as any).externalAccountBinding = this.createSignedHmacBody(eabHmacKey, url, jwk, { kid: eabKid });
            }
        }
        ...
        if (this.debug) {
            ngx.log(ngx.INFO, `njs-acme: [http] Signed request body: ${JSON.stringify(data)}`);
        }
        ...
        if (resp.status === 400) {
            ...
            if (respData?.type === 'urn:ietf:params:acme:error:badNonce' && (attempts < this.maxBadNonceRetries)) {
                ...
                ngx.log(ngx.WARN, `njs-acme: [http] Invalid nonce error, retrying (${attempts}/${this.maxBadNonceRetries}) signed request to: ${url}`);
     ...
     */
    async apiRequest(url: string, payload: any = null, validStatusCodes: number[] = [], { includeJwsKid = true, includeExternalAccountBinding = false } = {}) {
        const kid = includeJwsKid ? this.getAccountUrl() : null;
        if (this.debug) {
            ngx.log(ngx.INFO, `njs-acme: [http] Preparing a new api request kid=${kid}, payload=${JSON.stringify(payload)}`);
        }
        const resp = await this.signedRequest(url, payload, { kid, includeExternalAccountBinding });

        if (validStatusCodes.length && (validStatusCodes.indexOf(resp.status) === -1)) {
            const b = await resp.json()
            ngx.log(ngx.WARN, `njs-acme: [http] Received unexpected status code ${resp.status} for API request ${url}. Expected status codes: ${validStatusCodes.join(', ')}. Body response: ${JSON.stringify(b)}`);
    ...
    * @returns {Promise<object>} HTTP response
    */
    async apiResourceRequest(resource: AcmeResource, payload: any = null, validStatusCodes: number[] = [], { includeJwsKid = true, includeExternalAccountBinding = false } = {}) {
    ...
    async getDirectory() {
        if (!this.directory) {
            ...
            if (this.debug) {
                ngx.log(ngx.INFO, `njs-acme: [http] Fetched directory: ${JSON.stringify(this.directory)}`);
            }
        }
    }
    ...
    async getJwk() {
        ...
        if (!this.jwk) {
            if (this.debug) {
                ngx.log(ngx.INFO, 'njs-acme: [http] Public JWK not set. Obtaining it from Account Private Key...');
            }
            this.jwk = await getPublicJwk(this.accountKey);
            if (this.debug) {
                ngx.log(ngx.INFO, `njs-acme: [http] Obtained Account Public JWK: ${JSON.stringify(this.jwk)}`);
            }
        }
        return this.jwk;
    }
    ...
    async getNonce() {
        ...
        if (!resp.headers.get('replay-nonce')) {
            ngx.log(ngx.ERR, 'njs-acme: [http] No nonce from ACME provider. "replay-nonce" header found');
    ...
    async getResourceUrl(resource: AcmeResource): Promise<string> {
        ...
        if (this.directory != null && !this.directory[resource]) {
            ngx.log(ngx.ERR, `njs-acme: [http] Unable to locate API resource URL in ACME directory: "${resource}"`);
    ...
    async getMetaField(field: string): Promise<string | undefined> {
        await this.getDirectory();
        if (this.directory && 'meta' in this.directory && (field in this.directory.meta)) {
     ...
     */
    prepareSignedBody(alg: AcmeSignAlgo | string, url: NjsStringLike, payload = null, jwk: RsaPublicJwk | EcdsaPublicJwk | null | undefined, { nonce = null, kid = null } = {}): SignedPayload {
     ...
     */
    async createSignedHmacBody(hmacKey: string, url: string, payload = null, { nonce = null, kid = null } = {}) {
        if (!hmacKey) {
            throw new Error("HMAC key is required.");
        }
        const result = this.prepareSignedBody('HS256', url, payload, { nonce, kid });
        var h = require('crypto').createHmac('sha256', Buffer.from(hmacKey, 'base64'));
     ...
     */
    async createSignedBody(url: NjsStringLike, payload: any = null, { nonce = null, kid = null } = {}): Promise<SignedPayload> {
        const jwk = this.jwk!;
        ...
        const result = this.prepareSignedBody(headerAlg, url, payload, jwk, { nonce, kid });

        if (this.debug) {
            ngx.log(ngx.INFO, `njs-acme: [http] Prepared signed payload ${JSON.stringify(result)}`);
        }

        let sign;
        if (jwk.kty === 'EC') {
            const hash = await crypto.subtle.digest({ name: signerAlg }, `${result.protected}.${result.payload}`);
            sign = await crypto.subtle.sign({
                name: "ECDSA",
                hash: hash
            }, this.accountKey, hash);
        } else {
            sign = await crypto.subtle.sign({ "name": "RSASSA-PKCS1-v1_5" }, this.accountKey, `${result.protected}.${result.payload}`);
        }
     ...
     * @returns {Promise<object>} HTTP response
     */
    updateAuthorization(url: string, data): Promise<Response> {
        return this.apiRequest(url, data, [200]);
    }
