...
function asn1_parse_integer(buf) {
  ...
  if (is_negative) return value - compl_int
  else return value
}
...
function asn1_parse_ia5_string(buf) {
  if (is_ia5(buf)) return buf.toString()
  else throw 'Not a IA5String: ' + buf
}
...
function asn1_read(buf) {
  ...
  while (pointer < buf.length) {
    ...
    if (pointer + length > buf.length)
      throw (
        'length exceeds buf side: ' + length + ' ' + pointer + ' ' + buf.length
      )
    ...
    if (is_constructed) {
      ...
      a.push(
        asn1_parse_primitive(
          tag_class,
          tag,
          buf.slice(pointer, pointer + length)
        )
      )
    }
